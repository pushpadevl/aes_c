...
void decrypt(uch *state, uch *expandedKey){
    ...
    int step = AES_ROUNDS*16;
    uch roundKey[16];
    

    memcpy(roundKey, expandedKey+step, 16);
    addRoundKey(state, roundKey);                   //step 4
    invShiftRows(state);                            //step 2
    for(int j=0;j<4;j++) invSubBytes(state+j*4);    //step 1
    step-=16;
    printf("Round-%d\n",AES_ROUNDS);
    for(int j=0;j<16;j++) printf("0x%02x ", state[j]);
    printf("\n\n");
...
void invShiftRows(uch *state){  
    for(int i=1;i<4;i++){
        ...
        uch tmp;
        while(j++<=i){
            tmp = state[i+12];      //try memcpy here, see if it works
...
void invMixColumns(uch *state){
    ...
    for(int i=0;i<16;i+=4){
        ...
        state[i] = prodTable[6][a] ^ prodTable[4][b] ^ prodTable[5][c] ^ prodTable[3][d]; 
        state[i+1] = prodTable[3][a] ^ prodTable[6][b] ^ prodTable[4][c] ^ prodTable[5][d]; 
        state[i+2] = prodTable[5][a] ^ prodTable[3][b] ^ prodTable[6][c] ^ prodTable[4][d]; 
        state[i+3] = prodTable[4][a] ^ prodTable[5][b] ^ prodTable[3][c] ^ prodTable[6][d]; 
    }
}
