...
#define Nb 4 //block size 4 words for all AES-128,192,256
#define Nk 4 //4 words = 16 bytes key length for AES128, for 192, 256, it shd be 6,8 resp.
#if Nk==4
...
const uch mixBox[16] = {0x02,0x03,0x01,0x01,0x01,0x02,0x03,0x01,0x01,0x01,0x02,0x03,0x03,0x01,0x01,0x02};
...
uch multiplyModPx(uch a, uch b);
void main(){
    uch sec_key[4*Nk] = {0x2b, 0x7e, 0x15 , 0x16 , 0x28 , 0xae , 0xd2 , 0xa6 , 0xab , 0xf7 , 0x15 , 0x88 , 0x09 , 0xcf , 0x4f , 0x3c};
    ...
    //for(int i=0;i<EKSIZE;i++){ printf("%d => ",i); for(int j=0;j<4;j++) printf("%02x ", expandedKey[i*4+j]); printf("\n");}

    uch state[16]={0x32, 0x43 , 0xf6 , 0xa8 , 0x88 , 0x5a , 0x30 , 0x8d , 0x31 , 0x31 , 0x98 , 0xa2 , 0xe0 , 0x37 , 0x07 , 0x34};
    //uch state[16];
    encrypt(state, expandedKey);
    //uch state[16]={0xd4, 0xbf, 0x5d, 0x30, 0xe0, 0xb4, 0x52, 0xae, 0xb8, 0x41, 0x11, 0xf1, 0x1e, 0x27, 0x98, 0xe5};
    //mixColumns(state); 
...
void encrypt(uch *state, uch *expandedKey){ //for 128-bit AES only, need examples
    ...
    //for(int j=0;j<16;j++) printf("0x%02x ", roundKey[j]);
    //printf("\n");
    ...
    for(int i=1;i<=AES_ROUNDS;i++){
        ...
        if(i!=AES_ROUNDS) mixColumns(state);
        //for(int j=0;j<16;j++) printf("0x%02x ", state[j]);
        //printf("\n");
        //memcpy(roundKey, expandedKey+i*16, 16);
        memcpy(roundKey, expandedKey + i*Nk*4, 16);
        addRoundKey(state,roundKey);
        for(int j=0;j<16;j++) printf("0x%02x ", state[j]);
        printf("\n\n");
    }
...
uch multiplyModPx(uch a, uch b){        
    ...
    */ 
    ...
    for (int counter = 0; counter < 8; counter++) {
        ...
        hi_bit_set = (a & 0x80);            // hbs = 1000 0000, same
        a <<= 1;                            // a = 1011 1110, 0111 1100
        if (hi_bit_set == 0x80) {           // true
            a ^= 0x1B;                      // 0x1B is the irr poly for GF(2^8)
                                            // 1011 1110 ^ 0001 1011 = 1010 0101 = a
        }
        b >>= 1;                            // b = 0000 0001
    }
    return p;
