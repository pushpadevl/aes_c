...
void precomp(uch table[7][256]){
    ...
    table[5][0] = 0;
    table[6][0] = 0;
    for(uch i=0x1;i!=0;i++){        //condition to avoid wraparound
        table[0][i] = i;
        table[1][i] = multiplyModPx(i,two);
        table[2][i] = multiplyModPx(i,thr);
        table[3][i] = multiplyModPx(i,nin);
        table[4][i] = multiplyModPx(i,bee);
        table[5][i] = multiplyModPx(i,dee);
        table[6][i] = multiplyModPx(i,ee);
    }
    //printf("1\t 2 \t 3\n");
    
    for(uch i=3;i<7; i++){
        printf("0x%02x, ", table[i][0]);
        for(uch j=1;j!=0;j++){
